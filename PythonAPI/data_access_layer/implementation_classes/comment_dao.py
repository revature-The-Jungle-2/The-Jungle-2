...
class CommentDAOImp(CommentDAO):
    ...
    def create_comment(self, post_id: int, user_id: int, comment_text: str, group_id: int, reply_user: int) -> Comment:
        ...
        sql = "insert into "+schema_prefix+"comment_table values(default, %s, %s, %s, %s, %s, %s, default) returning comment_id "
        cursor = connection.cursor()
        ...
        sql = "select * "+schema_prefix+"from comment_table where comment_id = %s"
        cursor = connection.cursor()
    ...
    def get_comment_by_post_id(self, post_id: int) -> List[ReturnedComment]:
        ...
        sql = "select ct.comment_id, ct.post_id, ct.user_id , ct.group_id, ct.username, ct.comment_text, ct.likes, " \
              "ct.date_time_of_creation, ut.username from "+schema_prefix+"comment_table as ct inner join "+schema_prefix+"user_table as ut " \
    ...
    def delete_comment(self, comment_id: int) -> bool:
        ...
        sql = f"select * from "+schema_prefix+"comment_table where comment_id = %(comment_id)s;"
        cursor = connection.cursor()
        cursor.execute(sql, {"comment_id": comment_id})
        if not cursor.fetchone():
            raise CommentNotFound("Comment Not Found")

        sql = "delete from "+schema_prefix+"comment_table where comment_id = %s"
        cursor = connection.cursor()
