...
class LikePostDaoImp(LikePostDAO):
    def like_post(self,post_id:int):
            sql = "update "+schema_prefix+"post_table set likes = likes + 1 where post_id=%s returning likes"
    ...
    def like_comment(self, comment_id: int):
            sql = "update "+schema_prefix+"comment_table set likes = likes + 1 where comment_id=%s returning likes"
            cursor = connection.cursor()
